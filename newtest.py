...
import asyncio
import os
from fuzzywuzzy import fuzz  # Changed from rapidfuzz to fuzzywuzzy
...
TOKEN = "7413228285:AAHMeYM5yIC-n7lsWclXgZ8ESpclZZ_Aljg"
...
FUZZY_MATCH_THRESHOLD = 80  # Minimum match percentage for fuzzy search
...
def get_all_captions():
    """Get all captions from cache for fuzzy matching"""
...
def fuzzy_search(query, choices, threshold=FUZZY_MATCH_THRESHOLD):
    """Find best fuzzy matches using fuzzywuzzy"""
    results = process.extract(query, choices, scorer=fuzz.token_set_ratio, limit=None)
...
def search_multiple_matches(user_query: str):
    ...
    if not results:
        fuzzy_results = fuzzy_search(user_query, all_captions)
        if fuzzy_results:
            best_match_text = fuzzy_results[0][0]
            with open(CACHE_FILE, "r", encoding="utf-8") as f:
                for line in reversed(f.readlines()):
                    try:
                        msg_id, caption = line.strip().split("||", 1)
                        if caption == best_match_text:
                            results.append((int(msg_id), caption))
                            break
                    except:
                        continue
...
async def upload_caches(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "Please upload the new cache file (message_cache.txt). I will replace the existing one."
    )
...
async def upload_ids(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "Please upload the new user IDs file (id.txt). I will replace the existing one."
    )
